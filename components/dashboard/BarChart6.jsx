...
const BarChart6 = (props) => {
  ...
  const getDistributeurList = (data) => {
    let distList = [];
    data.map((dist) => {
      distList.push(dist.nom);
    });

    return distList;
  };

  const getRevunues = (data) => {
    let revenues = [];

    data.map((dist) => {
      revenues.push(dist.revenu);
    });
  ...
  useEffect(() => {
    const colors = [`rgb(${props.color})`, "rgb(237,125,49)"];

    props.data &&
      setChartData({
        labels: getDistributeurList(props.data),
        datasets: [
          {
            label: "Bar 1",
            data: getRevunues(props.data), // First value of each array (Bar 1)
